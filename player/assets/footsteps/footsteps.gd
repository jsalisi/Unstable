...
func _process(delta):
	if $Timer.is_stopped():
		...
		or Input.is_action_pressed("move_right")) and player.is_on_floor(): # If we move on the floor play the sound and adjust the footstep rate
			$Timer.wait_time = clamp(timer_speed / 1, 0.5/1.6, 0.5)
			play_sound(-20 / clamp(1, 0.5, 1.5))
			$Timer.start()
